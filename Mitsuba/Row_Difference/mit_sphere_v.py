...
def make_scene(cube_z: float, sphere_z: float) -> mi.Scene:
    ...
    wall_z = float(1.3)
    scene_dict = {'type': 'scene', 'integrator': {'type': 'path'}, 'sensor': {'type': 'perspective', 'to_world': mi.ScalarTransform4f.look_at(origin=CAM_ORIGIN, target=CAM_TARGET, up=CAM_UP), 'fov': CAM_FOV, 'film': {'type': 'hdrfilm', 'width': IMG_W, 'height': IMG_H, 'rfilter': {'type': 'tent'}, 'pixel_format': 'rgb'}, 'sampler': {'type': 'multijitter', 'sample_count': SPP}}, 'env_emitter': {'type': 'constant', 'radiance': {'type': 'rgb', 'value': [0.1, 0.1, 0.1]}}, 'sphere': {'type': 'sphere', 'to_world': mi.ScalarTransform4f.translate([0, 0, cube_z]) @ mi.ScalarTransform4f.scale(0.15), 'bsdf': {'type': 'roughplastic', 'alpha': 1.0, 'diffuse_reflectance': {'type': 'rgb', 'value': [0.95, 0.72, 0.55]}, 'specular_reflectance': {'type': 'rgb', 'value': [0.01, 0.01, 0.01]}}}}
