...
def make_scene(cube_z: float, sphere_z: float) -> mi.Scene:
    ...
    wall_z = float(1.3)
    scene_dict = {'type': 'scene', 'integrator': {'type': 'path'}, 'sensor': {'type': 'perspective', 'to_world': mi.ScalarTransform4f.look_at(origin=CAM_ORIGIN, target=CAM_TARGET, up=CAM_UP), 'fov': CAM_FOV, 'film': {'type': 'hdrfilm', 'width': IMG_W, 'height': IMG_H, 'rfilter': {'type': 'tent'}, 'pixel_format': 'rgb'}, 'sampler': {'type': 'multijitter', 'sample_count': SPP}}, 'env_emitter': {'type': 'constant', 'radiance': {'type': 'rgb', 'value': [0.8, 0.8, 0.8]}}, 'wall': {'type': 'rectangle', 'to_world': mi.ScalarTransform4f.translate([0.0, 1.2, wall_z]) @ mi.ScalarTransform4f.rotate([0, 1, 0], 180.0) @ mi.ScalarTransform4f.scale([5.0, 3.0, 1.0]), 'bsdf': {'type': 'roughplastic', 'alpha': 0.65, 'diffuse_reflectance': {'type': 'rgb', 'value': [0.65, 0.64, 0.62]}, 'specular_reflectance': {'type': 'rgb', 'value': [0.01, 0.01, 0.01]}}}, 'ground': {'type': 'rectangle', 'to_world': mi.ScalarTransform4f.translate([0.0, ground_y, 0.0]) @ mi.ScalarTransform4f.rotate([1, 0, 0], -90.0) @ mi.ScalarTransform4f.scale([5.0, 1.5, 5.0]), 'bsdf': {'type': 'roughplastic', 'alpha': 0.22, 'diffuse_reflectance': {'type': 'rgb', 'value': [0.36, 0.28, 0.22]}, 'specular_reflectance': {'type': 'rgb', 'value': [0.04, 0.04, 0.04]}}}, 'cube': {'type': 'cube', 'to_world': mi.ScalarTransform4f.translate([-0.12, cube_center_y, cube_z]) @ mi.ScalarTransform4f.rotate([0, 1, 0], CUBE_ROT_DEG1) @ mi.ScalarTransform4f.scale([CUBE_EDGE / 2.0, CUBE_EDGE / 0.7, CUBE_EDGE / 2.0]), 'bsdf': {'type': 'roughplastic', 'alpha': 0.9, 'diffuse_reflectance': {'type': 'rgb', 'value': [0.95, 0.72, 0.55]}, 'specular_reflectance': {'type': 'rgb', 'value': [0.03, 0.03, 0.03]}}}, 'sphere': {'type': 'cube', 'to_world': mi.ScalarTransform4f.translate([0.12, sphere_center_y, sphere_z]) @ mi.ScalarTransform4f.rotate([0, 1, 0], CUBE_ROT_DEG2) @ mi.ScalarTransform4f.scale([CUBE_EDGE / 1.7, CUBE_EDGE / 1.7, CUBE_EDGE / 2.0]), 'bsdf': {'type': 'roughplastic', 'alpha': 0.9, 'diffuse_reflectance': {'type': 'rgb', 'value': [0.95, 0.72, 0.55]}, 'specular_reflectance': {'type': 'rgb', 'value': [0.03, 0.03, 0.03]}}}}
...
def main():
    ...
    combined_tag = f'combined_w{IMG_W}x{IMG_H}_oddT{EXPOSURE_ODD * 1000.0:.1f}ms_evenT{EXPOSURE_EVEN * 1000.0:.1f}ms_v1{SPEED_OBJ1:.2f}_v2{SPEED_OBJ2:.2f}'
    ...
    try:
        run1 = run_odd
        run2 = run_even
        denom_old = float(prev_exposure + GAP_T)
        if denom_old != 0.0:
            vmap_old = (run2['depth_map'] - run1['depth_map']) / denom_old
            save_npy_png(vmap_old, os.path.join(OUT_DIR, 'velocity_map_run1_run2'), cmap='seismic', vmin=-10, vmax=10, title=f'Velocity Map v=(d2-d1)/(T+Δt), T={prev_exposure * 1000.0:.1f}ms, Δt={GAP_T * 1000.0:.1f}ms', cbar_label='Velocity (m/s)')
    except Exception:
        pass
