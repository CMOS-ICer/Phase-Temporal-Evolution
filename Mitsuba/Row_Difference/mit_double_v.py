...
VARIANT = 'cuda_ad_rgb'
OUT_DIR = 'result_double_v'
IMG_W, IMG_H = (512, 512)
SPP = 100
MOD_FREQ = 100000000.0
ROW_ODD_EXPOSURE = 0.008
ROW_EVEN_EXPOSURE = 0.08
NT = 100
OBJ1_INIT_Z = 0.8
OBJ2_INIT_Z = 0.8
SPEED_OBJ1 = -2.0
...
CUBE_ROT_DEG1 = 0
CUBE_ROT_DEG2 = 0
CAM_ORIGIN = [0.0, 0.15, 0.0]
CAM_TARGET = [0.0, 0.15, 2.0]
CAM_UP = [0.0, 2.0, 0.0]
CAM_FOV = 70.0
...
def make_scene(cube_z: float, sphere_z: float) -> mi.Scene:
    ...
    wall_z = float(1.3)
    scene_dict = {'type': 'scene', 'integrator': {'type': 'path'}, 'sensor': {'type': 'perspective', 'to_world': mi.ScalarTransform4f.look_at(origin=CAM_ORIGIN, target=CAM_TARGET, up=CAM_UP), 'fov': CAM_FOV, 'film': {'type': 'hdrfilm', 'width': IMG_W, 'height': IMG_H, 'rfilter': {'type': 'tent'}, 'pixel_format': 'rgb'}, 'sampler': {'type': 'multijitter', 'sample_count': SPP}}, 'env_emitter': {'type': 'constant', 'radiance': {'type': 'rgb', 'value': [0.8, 0.8, 0.8]}}, 'cube': {'type': 'cube', 'to_world': mi.ScalarTransform4f.translate([-0.12, cube_center_y, cube_z]) @ mi.ScalarTransform4f.rotate([0, 1, 0], CUBE_ROT_DEG1) @ mi.ScalarTransform4f.scale([CUBE_EDGE / 2.0, CUBE_EDGE / 0.7, CUBE_EDGE / 2.0]), 'bsdf': {'type': 'diffuse', 'reflectance': {'type': 'rgb', 'value': [0.95, 0.3, 0.3]}}}, 'sphere': {'type': 'cube', 'to_world': mi.ScalarTransform4f.translate([0.12, sphere_center_y, sphere_z]) @ mi.ScalarTransform4f.rotate([0, 1, 0], CUBE_ROT_DEG2) @ mi.ScalarTransform4f.scale([CUBE_EDGE / 1.7, CUBE_EDGE / 1.7, CUBE_EDGE / 1.5]), 'bsdf': {'type': 'diffuse', 'reflectance': {'type': 'rgb', 'value': [0.95, 0.3, 0.3]}}}}
...
def run_once(start_time: float, run_name: str, exposure: float):
    T = float(exposure)
    f = float(MOD_FREQ)
...
def save_run_outputs(run: dict):
    ...
    save_colorbar_vertical_png(depth, depth_base, cmap_name='viridis', vmin=0.1, vmax=0.9, cbar_label='Depth (m)', dpi=300, height_inches=6.0, width_inches=1.2)
    print(f'Saved depth images and colorbar for {tag}')

def main():
    ensure_dir(OUT_DIR)
    clear_dir_contents(OUT_DIR)
    run_odd = run_once(start_time=0.0, run_name='rows_odd', exposure=ROW_ODD_EXPOSURE)
    save_run_outputs(run_odd)
    run_even = run_once(start_time=0.0, run_name='rows_even', exposure=ROW_EVEN_EXPOSURE)
    save_run_outputs(run_even)
    depth_odd = run_odd['depth_map']
    depth_even = run_even['depth_map']
    H, W = depth_odd.shape
    combined_depth = np.full((H, W), np.nan, dtype=np.float32)
    for r in range(H):
        if r % 2 == 1:
            combined_depth[r, :] = depth_odd[r, :]
        else:
            combined_depth[r, :] = depth_even[r, :]
    comb_base = os.path.join(OUT_DIR, 'depth_combined_rows_odd8ms_even80ms')
    if output_allowed_path(comb_base + '.npy'):
        np.save(comb_base + '.npy', combined_depth)
    combined_for_png = np.copy(combined_depth)
    finite_mask_comb = np.isfinite(combined_for_png)
    if np.any(finite_mask_comb):
        combined_for_png[finite_mask_comb] = np.clip(combined_for_png[finite_mask_comb], 0.1, 0.9)
    save_image_no_cbar_exact_pixels(combined_for_png, comb_base, vmin=0.1, vmax=0.9, cmap_name='viridis')
    save_colorbar_vertical_png(combined_depth, comb_base, cmap_name='viridis', vmin=0.1, vmax=0.9, cbar_label='Depth (m)', dpi=300, height_inches=6.0, width_inches=1.2)
    print(f"Saved combined depth map: {comb_base + '.png'} and colorbar.")
    denom_half = (ROW_EVEN_EXPOSURE - ROW_ODD_EXPOSURE) / 2.0
    if abs(denom_half) < 1e-12:
        pass
    ...
    for i in range(H - 1):
        if i % 2 == 0:
            ...
            even_idx = i + 1
            odd_idx = i
        depth_even_row = combined_depth[even_idx, :]
        depth_odd_row = combined_depth[odd_idx, :]
        valid_mask = np.isfinite(depth_even_row) & np.isfinite(depth_odd_row)
        if np.any(valid_mask) and abs(denom_half) >= 1e-12:
            vmap_row = np.full((W,), np.nan, dtype=np.float32)
            vmap_row[valid_mask] = (depth_even_row[valid_mask] - depth_odd_row[valid_mask]) / denom_half
            vmap[i, :] = vmap_row
        else:
            vmap[i, :] = np.nan
    vel_base = os.path.join(OUT_DIR, 'velocity_rows_diff_formula')
    if output_allowed_path(vel_base + '.npy'):
        np.save(vel_base + '.npy', vmap)
    vmap_for_png = np.copy(vmap)
    ...
    canvas[0:H - 1, :] = vmap_for_png
    save_image_no_cbar_exact_pixels(canvas, vel_base, vmin=-10.0, vmax=10.0, cmap_name='seismic')
    save_colorbar_vertical_png(vmap, vel_base, cmap_name='seismic', vmin=-10.0, vmax=10.0, cbar_label='Velocity (m/s)', dpi=300, height_inches=6.0, width_inches=1.2)
